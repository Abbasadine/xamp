...
function pp_in_plugin_update_message( $plugin_data, $response ) {
	...
	if ( 'valid' !== $data->license ) {
		...
		$main_msg = sprintf( __( 'بروزرسانی کنید', 'powerpack' ), $data->license );

		$message  = '';
		$message .= '<p class="pp-update-message">';
		$message .= __( '<strong>آپدیت خودکار</strong>', 'powerpack' );
		$message .= '&nbsp;&nbsp;';
		$message .= $main_msg;
		$message .= ' <a href="https://elementor-site.ir" class="pp-buy-button" target="_blank">';
		$message .= __( 'المنتور سایت', 'powerpack' );
		$message .= ' &raquo;</a>';
		$message .= '</p>';
